...
n=`grep NORM_DENSITY ../../3-refinement-wat/1-Map-Preparation/log.preprocess | awk '{print $NF}'`
r=`grep Resolution ../../3-refinement-wat/1-Map-Preparation/log.preprocess | awk '{print $NF/10.0}'`
s=`grep BEST_SCALE ../../3-refinement-wat/3-Map-Scaling/BEST_SCALE | awk '{print $NF}'`
...
do
  ...
  # create tpr file
  gmx_mpi grompp -f 0-nvt-production.mdp -c rep-${dd}/conf.gro -n ../../3-refinement-wat/0-Building/index.ndx -p ../../3-refinement-wat/0-Building/topol.top -o rep-${dd}/production.tpr
